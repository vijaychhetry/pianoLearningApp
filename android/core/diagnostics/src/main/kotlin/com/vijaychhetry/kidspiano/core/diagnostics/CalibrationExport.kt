...
fun calibrationToJson(profile: CalibrationProfile): String {
    val keys = profile.notes.joinToString(",") { note ->
        """{"note":"${midiToNoteName(note.midiNote)}","midi":${note.midiNote},"expectedHz":${fmt(note.expectedFrequency)},"medianHz":${fmt(note.observedMedianFrequency)},"spreadCents":${fmt(note.frequencySpread)},"samples":${note.sampleCount},"confidence":${fmt(note.confidence)}}"""
    }
